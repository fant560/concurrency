package chapter6.filtering_stream_elements;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        persons.parallelStream().filter(p -> p.getSalary() < 30000).forEach(p -> System.out.printf("%s, %s\n", p.getLastName(), p.getFirstName()));

        Arrays.asList(numbers).parallelStream().mapToInt(n -> n).filter(n -> n < 2).forEach(n -> System.out.printf("%d\n", n));

        persons.parallelStream().mapToDouble(p -> p.getSalary()).sorted()
                .limit(5).forEach(s -> System.out.printf("Limit: %f \n", s));

        persons.parallelStream().mapToDouble(p -> p.getSalary())
                .sorted().skip(5).forEach(s -> System.out.printf("Skip: %f\n", s));
    }
